...
public class BDDPrinter {
	...
	public static ArrayList<String> printSetString(int bdd, int max, NodeTable nt, NodeName nn)  {
		ArrayList<String> r = new ArrayList<>();
		if( bdd < 2) {
			if(nn == null)
				if (bdd == 0) r.add("FALSE");
				else r.add("TRUE");
			else
			if (bdd == 0) r.add(nn.zero());
			else r.add(nn.one());
		} else {
			if(BDDPrinter.set_chars == null || BDDPrinter.set_chars.length < max)
				BDDPrinter.set_chars = Allocator.allocateCharArray(max);
			...
			r.addAll(printSet_rec_string(bdd, 0, nt, nn));
			helpGC();
		}
		return r;
	}
	...
	private static ArrayList<String> printSet_rec_string(int bdd, int level, NodeTable nt, NodeName nn) {
		ArrayList<String> r = new ArrayList<>();
		if(level == set_chars_len) {
		...
		if(var > level || bdd == 1 ) {
			set_chars[level] = '-';
			r.addAll(printSet_rec_string(bdd, level+1, nt, nn));
			return r;
		}
		...
		if(low != 0) {
			set_chars[level] = '0';
			r.addAll(printSet_rec_string(low, level+1, nt, nn));
		}

		if(high != 0) {
			set_chars[level] = '1';
			r.addAll(printSet_rec_string(high, level+1, nt, nn));
		}
		return r;
	}
}
